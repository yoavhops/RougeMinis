...
public class ObjectPoolManager : MonoBehaviour{
	...
	public static Transform Spawn(Transform objT, Vector3 pos, Quaternion rot, float activeDuration=-1){	Init();
	...
	public static GameObject Spawn(GameObject obj, Vector3 pos, Quaternion rot, float activeDuration=-1){		Init();
	...
	public static void Unspawn(Transform objT, float delay){ 	Init();
		instance.StartCoroutine(instance.UnspawnRoutine(objT.gameObject, delay));
	}
	public static void Unspawn(GameObject obj, float delay){ 	Init();
	...
	public static void Unspawn(GameObject obj){	Init();
		if(instance!=null){
			for(int i=0; i<instance.poolList.Count; i++){
				if(instance.poolList[i].Unspawn(obj)) return;
			}
		}
		Destroy(obj);
	}
